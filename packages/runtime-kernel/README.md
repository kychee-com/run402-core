...
It is a Developer Preview / single-node reference runtime component. Core functions are also Developer Preview trusted-local-code semantics: pre-bundled Node 22 artifacts, single-node scheduled functions from release manifests, no external npm dependency install, explicit resource limits, and no hostile-code isolation claim.
