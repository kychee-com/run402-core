...
Core now exposes the public contract for trusted local Node functions. The first subset is deliberately narrow: pre-bundled `node22` source refs, no external npm dependencies, route-manifest function targets, typed fail-closed errors, and machine-readable resource/isolation defaults.
