...
- verify trusted-code Node function apply, routed HTTP fidelity, direct invoke, durable function runs, single-node schedule trigger manual runs, auth gates, role gates, local secrets, request IDs, logs/diagnostics, redaction, no-op reapply, stale-plan rejection, unsupported dynamic features, cleanup reporting, and worker restart persistence
